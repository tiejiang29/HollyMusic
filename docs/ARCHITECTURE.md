...
3. 单次取址 15s 超时（防脚本挂起），全瀑布总预算 45s；
...
一个源，不再终止整条瀑布；三个值均可 env 覆盖（`SOURCE_URL_*`）。
...
不入账、本机网络故障期间（DNS/连不上）不记坏。**当前只观测不动作**——按 band 跳过源属于 3c。
注意账本天然只看得到"被轮到的源"：健康路径下 10 个源里通常只有 1-2 个有样本，这正是周测
（主动全矩阵探测）不可省略的原因。
